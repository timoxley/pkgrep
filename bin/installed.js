...
matchInstalled(dirname, toMatch, argv, function(err, found, matched) {
  if (err) throw err
  matched = matched || []
  found = found || []
  if (argv.filter) {
    ...
    found = found.filter(function(dep) {
      try {
        return filter(dep)
      } catch (e) {
        return false
  ...
  var resultTotal = found.length

  if (argv.json) {
    if (argv.format) {
      var outData = formatObject(found, argv.format)
      if (!argv.flatten) {
    ...
    ? renderTable(found, argv)
    : renderText(found, argv)
...
function renderText(found, options) {
  return found.map(function(dep) {
    dep = Object.create(dep)
    return template.compile(options.format, {delimiters: DELIMITERS}).render(dep)
  }).join('\n')
}

function formatObject(found, format) {
  var VARIABLE_TAG = '_v'
  ...
  var vars = names.map(function(item) {
    return {
      key: item.n,
  ...
  return found.map(function(dep) {
    ...
    vars.forEach(function(v) {
      obj[v.key] = he.decode(v.value.render(dep))
    })
    return obj
  })
}

function renderTable(found, options) {
  return columnify(formatObject(found, options.format))
}
...
function getNameAtVersionPath(dep) {
  return dep.name + '@' + dep.version + ' ' + dep.realPath
}

function getNameAtVersion(dep) {
