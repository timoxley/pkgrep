...
function possibleFormatKeys(dirname, options) {
  matchInstalled(dirname, [], options, function (err, installed) {
    if (err) throw err;
    var data = JSON.parse(stringify(installed[0].parent));
    data.dependencies = {};
